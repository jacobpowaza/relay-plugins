...
process.stdin.on("end", async () => {
  ...
  const line = JSON.stringify({ at: new Date().toISOString(), agent: "codex", event: "session-end", input: parsedInput });
  const previous = (() => {
  ...
  try {
    ...
    const checkpoint = core.checkpointFromSessionEndInput(parsedInput, "codex", link.boardId);
